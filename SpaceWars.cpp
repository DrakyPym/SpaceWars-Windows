...
{
	int a = amarillo;
	Posicionar1(x,y);   Colorear(a); printf("%c%c   ", 175, 187);
	Posicionar1(x,y+1); Colorear(a); printf("%c%c%c  ", 175, 187, 187); 
	Posicionar1(x,y+2); Colorear(a); printf(" %c%c%c%c", 204, 205 , 205, 175);
	Posicionar1(x,y+3); Colorear(a); printf("%c%c%c  ", 175, 188, 188);
	Posicionar1(x,y+4); Colorear(a); printf("%c%c   ", 175, 188);
}

void NAVE::Borrar()
{
	Posicionar1(x,y);    cout << "     ";
	Posicionar1(x, y+1); cout << "     ";
	Posicionar1(x, y+2); cout << "     ";
	Posicionar1(x, y+3); cout << "     ";
	Posicionar1(x, y+4); cout << "     ";
}
...
int ancho = Obtener_Ancho_Ventana();
	...
	if(kbhit())
	{	Borrar();
		if((GetAsyncKeyState(VK_LEFT) & 0x8000) && x > 4) x -= 2;
		if((GetAsyncKeyState(VK_RIGHT) & 0x8000) && x < ancho-7) x += 2;
		if((GetAsyncKeyState(VK_UP) & 0x8000) && y > 4) y--;
		if((GetAsyncKeyState(VK_DOWN)& 0x8000) && y < alto-5) y++;
        Pintar();
    }
...
void NAVE::Pintar_Corazones()
{
	...
	Posicionar1(64,2); printf("Vida");
	Posicionar1(70,2); printf("          ");
	
	for(int i = 0; i < corazones; i++)
	{
		Posicionar1(70+i,2); printf("%c", 3);
	}
}
...
void NAVE::morir()
{
	...
	{
		Borrar();
		Posicionar1(x,y);   Colorear(a); printf("     ");
		Posicionar1(x,y+1); Colorear(a); printf(" **  "); 
		Posicionar1(x,y+2); Colorear(a); printf("  ** ");
		Posicionar1(x,y+3); Colorear(a); printf(" **  ");
		Posicionar1(x,y+4); Colorear(a); printf("     ");
		Sleep(200);
...
{
	int a = rojo;
	Posicionar1(x,y);   Colorear(a); printf("   %c ", 201);
	Posicionar1(x,y+1); Colorear(a); printf(" %c%c%c ", 201, 254, 187); 
	Posicionar1(x,y+2); Colorear(a); printf("%c%c%c%c%c", 40, 254 , 254, 254, 41);
	Posicionar1(x,y+3); Colorear(a); printf(" %c%c%c ", 200, 254,  188);
	Posicionar1(x,y+4); Colorear(a); printf("   %c ", 200);
}
...
bool BALA::fuera() {
    int ancho = Obtener_Ancho_Ventana();
    if (x >= ancho - 4) return true; // La bala est� fuera si su posici�n x es mayor o igual al ancho de la ventana - 4
    return false;
}

bool BALA::fueraOvni() {
    int ancho = Obtener_Ancho_Ventana();
    if (x <= 5) return true; // La bala est� fuera si su posici�n x es mayor o igual al ancho de la ventana - 4
    return false;
}
...
void juego_O()
{
	...
	int ancho = Obtener_Ancho_Ventana();
    int alto = Obtener_Altura_Ventana();
	Colorear1(verde); Pintar_Limites();
	OcultarCursor();
...
struct Coordenadas_Botones{  //Estructura del boton
	int x;
	int y;
	char color;
};

void Colorear(int a){//FUNCION PARA CAMBIAR DE COLOR LOS CARACTERES
	int cl = a;
	HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
	color(hConsole, cl);
}
...
{	//LOGO	
	...
	for(int i = 0; i < Altura_Logo-5;i++){//Espacio
		for(int j = 0; j < Largo_Logo; j++){
			Posicionar(CoordenadaX_Logo + j, CoordenadaY_Logo + i); 
			Colorear(blanco); 
			printf("%c", Logo[i][j]);
	...
	for(int i = 5; i< Altura_Logo; i++) {// SPACE
		for(int j = 0; j < 40;j++){
			Posicionar(CoordenadaX_Logo + j, CoordenadaY_Logo + i); 
			Colorear(amarillo); 
			printf("%c", Logo[i][j]);			
	...
	for(int i = 5; i< Altura_Logo; i++){// WARS
		for(int j = 40; j < Largo_Logo;j++){
			Posicionar(CoordenadaX_Logo + j, CoordenadaY_Logo + i); 
			Colorear(rojo); 
			printf("%c", Logo[i][j]);			
...
void Mover_Volver(){
    ...
    do {
        if (kbhit()) {
            ...
            if (tecla == ESPACIO) { // Verificar si se presion� Espacio
